...
This is absolute ly beautiful. By typing these events in this way we are making TS be _context aware_ about the information each event actually needs 🔥.
...
TS is telling is that at _THIS_ point of our code, it can't actually tell if the event is an onboarding event, termination event or signed up event. So we need to do some narrowing down first ⬇️.
...
By asserting the type of `myEvent` is actually `signed-up`, we are _narrowing down_ the type of `myEvent` to `CompanyEvent & SignedUpEvent` and TS will be smart enough to realize that's a `SignedUp` event 😎.
